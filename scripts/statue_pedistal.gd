	...
	for y in range(dimensions.y):
		for x in range(dimensions.x):
			for z in range(dimensions.z):
				...
				newVoxelContainer.position = Vector3(x,y,z)*voxel_size
	voxelHolder.position = -Vector3(dimensions.x - 1, -dimensions.y+1/voxel_size, dimensions.z - 1) * voxel_size * 0.5
